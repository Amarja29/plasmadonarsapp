...
def check(email):
    url = " https://x30zq063ob.execute-api.us-east-1.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = " https://x30zq063ob.execute-api.us-east-1.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = " https://x30zq063ob.execute-api.us-east-1.amazonaws.com/plasma/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://x30zq063ob.execute-api.us-east-1.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    emailids=[]
    for i in a:
